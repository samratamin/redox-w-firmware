...
static uint8_t data_payload_num[NRF_GZLL_CONST_MAX_PAYLOAD_LENGTH];  ///< Placeholder for data payload received from host.
...
static bool init_ok, enable_ok, push_ok, pop_ok, packet_received_left, packet_received_right, packet_received_num;
...
int main(void)
{
    ...
    nrf_gzll_add_packet_to_tx_fifo(2, data_payload_num, TX_PAYLOAD_LENGTH);	
    ...
    while (true)
    {
        ...
        if (packet_received_left)
        {
            ...
            data_buffer[3] =  ((data_payload_left[1] & 1<<7) ? 1:0) << 0 |
            ...
            data_buffer[6] =  ((data_payload_left[2] & 1<<7) ? 1:0) << 0 |
            ...
            data_buffer[9] =  ((data_payload_left[3] & 1<<7) ? 1:0) << 0 |
            ...
            data_buffer[12] =  ((data_payload_left[4] & 1<<7) ? 1:0) << 0 |
        ...
        if (packet_received_right)
        {
            ...
            data_buffer[4] =  ((data_payload_right[1] & 1<<7) ? 1:0) << 0 |
            ...
            data_buffer[7] =  ((data_payload_right[2] & 1<<7) ? 1:0) << 0 |
            ...
            data_buffer[10] =  ((data_payload_right[3] & 1<<7) ? 1:0) << 0 |
            ...
            data_buffer[13] =  ((data_payload_right[4] & 1<<7) ? 1:0) << 0 |
	...
	if (packet_received_num)
        {
            packet_received_num = false;

            data_buffer[2] =  ((data_payload_num[0] & 1<<7) ? 1:0) << 0 |
                              ((data_payload_num[0] & 1<<6) ? 1:0) << 1 |
                              ((data_payload_num[0] & 1<<5) ? 1:0) << 2 |
                              ((data_payload_num[0] & 1<<4) ? 1:0) << 3 |
                              ((data_payload_num[0] & 1<<3) ? 1:0) << 4 |
                              ((data_payload_num[0] & 1<<2) ? 1:0) << 5 |
                              ((data_payload_num[0] & 1<<1) ? 1:0) << 6;

            data_buffer[5] =  ((data_payload_num[1] & 1<<7) ? 1:0) << 0 |
                              ((data_payload_num[1] & 1<<6) ? 1:0) << 1 |
                              ((data_payload_num[1] & 1<<5) ? 1:0) << 2 |
                              ((data_payload_num[1] & 1<<4) ? 1:0) << 3 |
                              ((data_payload_num[1] & 1<<3) ? 1:0) << 4 |
                              ((data_payload_num[1] & 1<<2) ? 1:0) << 5 |
                              ((data_payload_num[1] & 1<<1) ? 1:0) << 6;

            data_buffer[8] =  ((data_payload_num[2] & 1<<7) ? 1:0) << 0 |
                              ((data_payload_num[2] & 1<<6) ? 1:0) << 1 |
                              ((data_payload_num[2] & 1<<5) ? 1:0) << 2 |
                              ((data_payload_num[2] & 1<<4) ? 1:0) << 3 |
                              ((data_payload_num[2] & 1<<3) ? 1:0) << 4 |
                              ((data_payload_num[2] & 1<<2) ? 1:0) << 5 |
                              ((data_payload_num[2] & 1<<1) ? 1:0) << 6;

            data_buffer[11] =  ((data_payload_num[3] & 1<<7) ? 1:0) << 0 |
                              ((data_payload_num[3] & 1<<6) ? 1:0) << 1 |
                              ((data_payload_num[3] & 1<<5) ? 1:0) << 2 |
                              ((data_payload_num[3] & 1<<4) ? 1:0) << 3 |
                              ((data_payload_num[3] & 1<<3) ? 1:0) << 4 |
                              ((data_payload_num[3] & 1<<2) ? 1:0) << 5 |
                              ((data_payload_num[3] & 1<<1) ? 1:0) << 6;

            data_buffer[14] =  ((data_payload_num[4] & 1<<7) ? 1:0) << 0 |
                              ((data_payload_num[4] & 1<<6) ? 1:0) << 1 |
                              ((data_payload_num[4] & 1<<5) ? 1:0) << 2 |
                              ((data_payload_num[4] & 1<<4) ? 1:0) << 3 |
                              ((data_payload_num[4] & 1<<3) ? 1:0) << 4 |
                              ((data_payload_num[4] & 1<<2) ? 1:0) << 5 |
                              ((data_payload_num[4] & 1<<1) ? 1:0) << 6;
        }
...
void nrf_gzll_host_rx_data_ready(uint32_t pipe, nrf_gzll_host_rx_info_t rx_info)
{
    ...
    {
        packet_received_num = true;
        num_active = 0;
        // Pop packet and write first byte of the payload to the GPIO port.
        nrf_gzll_fetch_packet_from_rx_fifo(pipe, data_payload_num, &data_payload_length);
    }
